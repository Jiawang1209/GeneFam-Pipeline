...
write_overview_status <- function(status, package, version, note) {
  status_df <- data.frame(
    check = "ppi_overview_plot",
    status = status,
    package = package,
    version = version,
    note = note,
  ...
  write.table(status_df, overview_status_file, sep = "\t", quote = FALSE, row.names = FALSE)
}
...
if (!requireNamespace("ggNetView", quietly = TRUE)) {
  ...
  write_overview_status("missing_dependency", "ggNetView", "version_not_detected", "ggNetView is not installed; PPI overview plot was skipped.")
...
if (!requireNamespace("ggplot2", quietly = TRUE) || !requireNamespace("patchwork", quietly = TRUE)) {
  ...
  write_overview_status("missing_dependency", "ggplot2_or_patchwork", "version_not_detected", "Required plotting packages are missing; PPI overview plot was skipped.")
...
if (nrow(edges) == 0) {
  ...
  write_overview_status("no_edges", "ggNetView", as.character(utils::packageVersion("ggNetView")), "No PPI edges were available for overview plotting.")
...
write_overview_status(
  "ready_ggnetview_alias",
  "ggNetView",
  as.character(utils::packageVersion("ggNetView")),
  "Reference-named ppi.pdf/png are aliases of the ggNetView PPI visualization, per project visualization policy."
)
