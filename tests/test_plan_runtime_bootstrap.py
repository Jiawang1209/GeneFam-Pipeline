...
def test_build_bootstrap_plan_groups_missing_commands_into_actionable_steps():
    rows = [
        ...
        {"command": "FastTree", "status": "missing", "path": ""},
    ...
    assert "Missing commands: nextflow, docker, apptainer, hmmsearch, diamond, mafft, FastTree, iqtree2, meme" in plan["markdown"]
    assert "FastTree" in plan["markdown"]
