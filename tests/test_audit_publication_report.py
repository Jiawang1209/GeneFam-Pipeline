    ...
    _write_tsv(
        ...
        [
            [
                ...
                "QC tables record missing feature rows",
            ...
            [
                ...
                "QC tables record interaction evidence limits",
    ...
    final_report.write_text(
        "\n".join(
            [
                ...
                "- QC warnings / limitations: QC tables record missing feature rows",
                ...
                "- QC warnings / limitations: QC tables record interaction evidence limits",
...
def test_publication_report_audit_requires_report_index_png_variant_for_each_plot(tmp_path):
    ...
    report_index = tmp_path / "report/report_index.tsv"
    ...
    _write_tsv(
        ...
        [
            ["tree_features_pdf", str(tmp_path / "plots/tree_features.pdf"), "available", "Tree feature PDF"],
            ["tree_features_png", str(tmp_path / "plots/tree_features.png"), "available", "Tree feature PNG"],
        ],
    )
    _write_tsv(
        figure_interpretations,
    ...
    assert by_check["report_index_plot_variants"]["status"] == "failed"
    assert "tree_features:png:missing_file" in by_check["report_index_plot_variants"]["note"]
...
def test_publication_report_audit_cli_writes_markdown_and_tsv(tmp_path):
    ...
    _write_tsv(
        ...
        [[
            ...
            "Expansion or contraction signals are visible in the per-species member totals",
...
def test_publication_report_audit_requires_final_report_identity_matches_run_config(tmp_path):
    plot_manifest = tmp_path / "plot_manifest.tsv"
    figure_interpretations = tmp_path / "figure_interpretations.tsv"
    software_versions = tmp_path / "software_versions.tsv"
    final_report = tmp_path / "final_report.md"
    ...
    assert by_check["final_report_embeds_publication_sections"]["status"] == "failed"
    assert "report_identity:project.name:expected=current_project:actual=stale_project" in by_check[
        "final_report_embeds_publication_sections"
    ]["note"]
...
def test_publication_report_audit_rejects_instructional_observation_text(tmp_path):
    plot_manifest = tmp_path / "plot_manifest.tsv"
    figure_interpretations = tmp_path / "figure_interpretations.tsv"
    software_versions = tmp_path / "software_versions.tsv"
    final_report = tmp_path / "final_report.md"
    ...
    _write_tsv(
        ...
        [[
            ...
            "review expression matrix completeness",
    ...
    final_report.write_text(
        "\n".join(
            [
                ...
                "| figure_key | plot_path | interpretation_status | qc_tables | method_and_software | reproducibility |",
                "| --- | --- | --- | --- | --- | --- |",
                "| expression_heatmap | plots/expression_heatmap.pdf | figure-specific close reading | tables/expression_gene_summary.tsv | plot_expression_heatmap.R; /usr/local/bin/R | python bin/genefam/run_expression_heatmap_smoke.py --r-bin /usr/local/bin/R --outdir results/expression_heatmap_smoke |",
                "## Figure Result Interpretations",
                "### expression_heatmap: Expression heatmap",
                "- Input data: family expression matrix",
                "- What the figure shows: expression clusters",
                "- Key observations: Inspect co-expression clusters, treatment-specific responses, and low-expression genes.",
                "- Biological interpretation: Clustered expression patterns can suggest functional divergence or conserved regulatory responses.",
                "- QC warnings / limitations: review expression matrix completeness",
    ...
    assert by_check["figure_interpretation_close_reading_voice"]["status"] == "failed"
    assert "expression_heatmap:key_observations:instructional_text" in by_check[
        "figure_interpretation_close_reading_voice"
    ]["note"]


def test_publication_report_audit_rejects_instructional_qc_and_status_text(tmp_path):
    plot_manifest = tmp_path / "plot_manifest.tsv"
    figure_interpretations = tmp_path / "figure_interpretations.tsv"
    software_versions = tmp_path / "software_versions.tsv"
    final_report = tmp_path / "final_report.md"
    ...
    _write_tsv(
        ...
        [[
            ...
            "clades share feature architecture",
            "conserved clade features support structural conservation",
            "Review missing feature rows before interpretation",
            "tables/tree_feature_matrix.tsv",
            "FastTree; plot_tree_features.R; /usr/local/bin/R",
            "python bin/genefam/run_tree_feature_smoke.py --r-bin /usr/local/bin/R --outdir results/tree_feature_smoke",
            "figure-specific close reading; validate topology before manuscript use",
            "plots/tree_features.pdf",
        ]],
    )
    _write_tsv(
    ...
    final_report.write_text(
        "\n".join(
            [
                ...
                "| figure_key | plot_path | interpretation_status | qc_tables | method_and_software | reproducibility |",
                "| --- | --- | --- | --- | --- | --- |",
                "| tree_features | plots/tree_features.pdf | figure-specific close reading; validate topology before manuscript use | tables/tree_feature_matrix.tsv | FastTree; plot_tree_features.R; /usr/local/bin/R | python bin/genefam/run_tree_feature_smoke.py --r-bin /usr/local/bin/R --outdir results/tree_feature_smoke |",
                "## Figure Result Interpretations",
                "### tree_features: Tree features",
                "- Input data: tree and feature tables",
                "- What the figure shows: tree-ordered feature tracks",
                "- Key observations: clades share feature architecture",
                "- Biological interpretation: conserved clade features support structural conservation",
                "- QC warnings / limitations: Review missing feature rows before interpretation",
                "- QC tables: tables/tree_feature_matrix.tsv",
                "- Method/software: FastTree; plot_tree_features.R; /usr/local/bin/R",
                "- Reproducibility: python bin/genefam/run_tree_feature_smoke.py --r-bin /usr/local/bin/R --outdir results/tree_feature_smoke",
                "- Result reading status: figure-specific close reading; validate topology before manuscript use",
                "- Output path: `plots/tree_features.pdf`",
            ]
        ),
        encoding="utf-8",
    ...
    assert by_check["figure_interpretation_close_reading_voice"]["status"] == "failed"
    assert "tree_features:qc_warnings:instructional_text" in by_check[
        "figure_interpretation_close_reading_voice"
    ]["note"]
    assert "tree_features:result_reading_status:instructional_text" in by_check[
        "figure_interpretation_close_reading_voice"
    ]["note"]
