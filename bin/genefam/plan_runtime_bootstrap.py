...
def build_bootstrap_plan(rows: list[dict[str, str]]) -> dict[str, str]:
    ...
    shell_lines = [
        ...
        "else",
        '  echo "[GeneFam] docker not found; skipping Docker image build and Docker profile smoke."',
        "fi",
        "if command -v apptainer >/dev/null 2>&1; then",
        "  if command -v docker >/dev/null 2>&1; then",
        "    apptainer build --force genefam-pipeline_latest.sif docker-daemon://genefam-pipeline:latest",
        "  else",
        '    echo "[GeneFam] docker not found; skipping docker-daemon SIF build."',
        "  fi",
        ...
        "else",
        '  echo "[GeneFam] apptainer not found; skipping Apptainer SIF builds and Apptainer profile smoke."',
        "fi",
    ...
    markdown = "\n".join(
        [
            ...
            "The generated shell checks whether `docker` and `apptainer` are available before running container-specific commands.",
            "",
            "## Verification",
            "",
