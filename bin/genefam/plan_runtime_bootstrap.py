...
def build_bootstrap_plan(rows: list[dict[str, str]]) -> dict[str, str]:
    ...
    markdown = "\n".join(
        [
            ...
            "## Primary Path",
            "",
            "Refresh the shared `GeneFamilyFlow` Conda environment from `envs/GeneFamilyFlow.conda.yaml`.",
            "The environment includes Nextflow, OpenJDK, R packages, HMMER, DIAMOND, MAFFT, IQ-TREE, MEME, and related helpers.",
            "",
            "```bash",
            "conda env update -n GeneFamilyFlow -f envs/GeneFamilyFlow.conda.yaml --prune",
            "```",
