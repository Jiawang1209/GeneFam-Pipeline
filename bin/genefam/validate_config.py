...
def validate_config(config: dict[str, Any], check_paths: bool = False, base_dir: Path | None = None) -> list[str]:
    ...
    if reference_generation and reference_generation.get("enabled") is not False:
        if reference_generation.get("source") != "tair_all_domains":
            errors.append("reference_generation.source must be tair_all_domains")
        if not reference_generation.get("domain_annotation"):
            errors.append("reference_generation.domain_annotation is required")
        if not reference_generation.get("reference_species"):
            errors.append("reference_generation.reference_species is required")
        domain_annotation = str(reference_generation.get("domain_annotation", ""))
        if check_paths and domain_annotation and not _path_exists(domain_annotation, base_dir):
            errors.append(f"reference_generation.domain_annotation path does not exist: {domain_annotation}")
        if not (gene_family.get("hmm_profiles", []) or []):
            errors.append("reference_generation requires gene_family.hmm_profiles")
