...
def run_promoter_cis_smoke(r_bin: str, outdir: Path) -> dict[str, Path]:
    ...
    subprocess.run(
        [
            ...
            str(outputs["promoter_cis_gene_element_matrix"]),
            str(outputs["promoter_cis_element_annotations"]),
            str(plot_dir),
        ],
        check=True,
    )
    ...
    summary_md.write_text(
        "\n".join(
            [
                ...
                f"Gene-element matrix: `{outputs['promoter_cis_gene_element_matrix']}`",
