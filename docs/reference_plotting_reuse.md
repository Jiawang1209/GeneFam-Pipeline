...
| MCScanX self/circlize intra-species duplication | `Reference/Long_Weixiong_20240323_1_GDSL/R/9.Circos_*.R` | `build_mcscanx_self_inputs.py`; `run_mcscanx_self.py`; `build_circlize_inputs.py`; `scripts/plot_mcscanx_circlize.R`; `PLOT_MCSCANX_SELF_CIRCOS`; `tables/circlize_link_density.tsv`; `tables/circlize_duplicate_type_tracks.tsv`; `plots/mcscanx_circlize.pdf/png` | done | Circlize output now uses MCScanX self pairs for each species, includes chromosome sectors, intra-species duplicated-gene links, linked-gene density windows, and duplicate-type track rows. |
