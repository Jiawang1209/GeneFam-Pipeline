...
| promoter cis-element | `Reference/Long_Weixiong_20240323_1_GDSL/R/10.promoter.R` | `extract_promoters.py`; `build_promoter_cis_elements.py`; `scripts/plot_promoter_cis_elements.R`; `PLOT_PROMOTER_CIS_ELEMENTS`; `tables/promoter_cis_gene_element_matrix.tsv`; `tables/promoter_cis_element_annotations.tsv`; `plots/promoter_cis_elements.pdf/png` | done | PlantCARE-style category matrix, top-element summary, per-element biological annotations, position summaries, and gene-by-element dot heatmap now exist. |
| PPI network | `Reference/Long_Weixiong_20240323_1_GDSL/R/11.ppi.R` | `build_ppi_tables.py`; `scripts/plot_ppi_ggnetview.R`; `PLOT_PPI_GGNETVIEW`; `tables/ppi_input_evidence.tsv`; `tables/ppi_network_qc.tsv`; `plots/ppi_ggnetview.pdf/png` | done | |
...
| Large-scale copy number and expansion | `Reference/Large-Scale Plant Genomic Identification and Analysis Uncover ASMT:COMT Copy Number Variation Driving Melatonin Dosage Balance./source_code/Figure_1C_1052sp.py`; `Figure_4B_Copy_Number.tsv`; `Figure_5B_Copy_Number.py` | `build_gene_family_info.py`; `scripts/plot_gene_family_info.R`; `tables/gene_family_pangenome_summary.tsv`; standard report copy-number tables and plots | partial | Copy-number class, expansion, and pan/core/dispensable summary tables now exist; species-tree ordered large-scale copy-number plots and ultra-large species-label layout still need refinement. |
| Large-scale Ks and pangenome comparison | `Reference/Large-Scale Plant Genomic Identification and Analysis Uncover ASMT:COMT Copy Number Variation Driving Melatonin Dosage Balance./source_code/Figure_2B_5F_Ks.py`; `Figure_3D_Core_Dispensable.py` | WGD raw handoff, Ks outputs, `tables/gene_family_pangenome_summary.tsv`, and figure interpretation templates | partial | Need combined Ks-by-pangenome-class comparison plots when accession-level core/dispensable input tables are provided. |
