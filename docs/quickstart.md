...
The wrapper also writes `results/local_acceptance/local_acceptance_summary.tsv` and `results/local_acceptance/local_acceptance_summary.md`, which record the release gate, `publication_report_audit`, report-index audit exit status, quickstart, delivery-bundle exit status, and `final_stage_blocker` in one place. On machines without Docker/Apptainer, `Overall status: blocked` means the analysis-flow evidence is release-ready but the final container packaging stage still needs the runtime unblock path. If R plotting checks fail, open `results/r_runtime_health/r_runtime_health.md` first; `R runtime health` is run before R visualization smokes to show whether `/usr/local/bin/R` itself can start. The publication audit points to `results/publication_report_audit/publication_report_audit.md` and verifies paper-style report closure: valid plot file signatures, `report_index_plot_variants` PDF/PNG plot variants, registered-only figure interpretation scope, plot manifest and interpretation output path consistency, embedded PNG previews in `final_report.md`, complete per-figure close-reading text, result-statement interpretation narratives, including QC warnings and reading status, rather than instructional prompts, `figure_interpretation_qc_specificity` checks for figure-specific QC warnings, QC tables and warnings, software/R package versions, parseable detected version values, `final_report_methods_summary` Methods Summary coverage, per-figure method/software version coverage, and reproducibility commands. The `Reference visual alignment audit` writes `results/reference_visual_alignment/reference_visual_alignment.md` and records the `reference_visual_alignment` delivery row after checking standard and WGD plot manifests for `tree_features, motif, gene structure, domain`, `MCScanX/synteny/circlize`, promoter cis-elements, expression heatmap, ggNetView PPI, and `Ka/Ks/WGD gamma beta alpha theta` coverage.
...
The release gate writes the final delivery index after the objective audit. It collects species-bank and manifest-mode input entrypoints, the manifest-mode standard DSL2 smoke evidence, the standard final report, prepared WGD report, publication report audit, report-index closure, alpha/beta/gamma/theta event evidence, runtime status, `r_runtime_health` evidence, documentation entrypoints, and a global paper-level figure gallery.

The delivery bundle now records both report-closure audits:

- `publication_report_audit`: standard report paper-style report closure at `results/publication_report_audit/publication_report_audit.md`, covering valid plot file signatures, `report_index_plot_variants` PDF/PNG plot variants, registered-only figure interpretation scope, plot manifest and interpretation output path consistency, embedded PNG previews in `final_report.md`, complete per-figure close-reading text, result-statement interpretation narratives, including QC warnings and reading status, rather than instructional prompts, `figure_interpretation_qc_specificity` checks for figure-specific QC warnings, QC tables and warnings, software/R package versions, parseable detected version values, visible `version_not_detected` rows, `final_report_methods_summary` Methods Summary coverage, per-figure method/software version coverage, and reproducibility commands
- `wgd_publication_report_audit`: WGD report closure at `results/publication_report_audit/wgd_publication_report_audit.md`, covering valid plot file signatures, `report_index_plot_variants` PDF/PNG plot variants, registered-only figure interpretation scope, plot manifest and interpretation output path consistency, embedded PNG previews in `final_report.md`, complete Ka/Ks/WGD figure close-reading text, result-statement interpretation narratives, including QC warnings and reading status, rather than instructional prompts, `figure_interpretation_qc_specificity` checks for figure-specific QC warnings, gamma beta alpha theta interpretation, QC tables and warnings, software/R package versions, parseable detected version values, visible `version_not_detected` rows, `final_report_methods_summary` Methods Summary coverage, per-figure method/software version coverage, and reproducibility commands
- `standard_report_index_audit`: standard report-index closure at `results/report_index_audit/standard_report_index_audit.md`, covering indexed plot manifests, software versions, `figure_interpretations.tsv`, `figure_interpretations.md`, `final_report.md`, `figure_traceability_matrix`, all available indexed report paths, and the required `final_report.md#figure-traceability-matrix` anchor
- `wgd_report_index_audit`: WGD report-index closure at `results/report_index_audit/wgd_report_index_audit.md`, covering indexed plot manifests, software versions, `figure_interpretations.tsv`, `figure_interpretations.md`, `final_report.md`, `figure_traceability_matrix`, all available indexed report paths, and the required `final_report.md#figure-traceability-matrix` anchor
- `r_runtime_health`: `/usr/local/bin/R` startup health at `results/r_runtime_health/r_runtime_health.md`, recorded in `results/delivery_bundle/delivery_manifest.tsv` before users inspect R-dependent plots
- `reference_visual_alignment`: Reference paper-level visual alignment at `results/reference_visual_alignment/reference_visual_alignment.md`, covering `standard_reference_visual_modules`, `wgd_reference_visual_modules`, and `reference_visual_interpretations` for family information, `tree_features, motif, gene structure, domain`, `MCScanX/synteny/circlize`, promoter cis-elements, expression heatmap, ggNetView PPI, and `Ka/Ks/WGD gamma beta alpha theta`
...
Each gallery row links PDF and PNG plot targets through `plot_path` and `plot_png_path`, plus its `figure_interpretations.md#plot_key` per-figure close-reading anchor, `software_versions.tsv`, `final_report.md`, and `figure_traceability_matrix` anchor. The Markdown gallery uses clickable links for those targets. The `figure_gallery_audit` step also checks valid gallery plot file signatures for PDF/PNG/SVG targets, clickable Markdown links, and runs `figure_gallery_traceability_targets` to require each traceability anchor to resolve to that row's `final_report.md#figure-traceability-matrix`, so a broken or placeholder plot file cannot pass merely because the path exists.
