...
Each gallery row links PDF and PNG plot targets through `plot_path` and `plot_png_path`, plus its `figure_interpretations.md#plot_key` per-figure close-reading anchor, `software_versions.tsv`, `final_report.md`, and `figure_traceability_matrix` anchor. The `figure_gallery_audit` step also checks valid gallery plot file signatures for PDF/PNG/SVG targets and runs `figure_gallery_traceability_targets` to require each traceability anchor to resolve to that row's `final_report.md#figure-traceability-matrix`, so a broken or placeholder plot file cannot pass merely because the path exists.
