...
python bin/genefam/run_nextflow_standard_smoke.py \
  ...
  --run-promoter-cis \
  --promoter-cis-elements tests/fixtures/promoter_cis/plantcare.tsv \
  --run-ppi \
  --ppi-edges tests/fixtures/ppi/ppi_edges.tsv \
  --ppi-nodes tests/fixtures/ppi/ppi_nodes.tsv \
  --expression-matrix tests/fixtures/expression/family_expression.tsv \
  --expression-metadata tests/fixtures/expression/sample_metadata.tsv \
